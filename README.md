...
- [ ] v1.1.0: Add new instructions for label manipulation
  ...
  - `RFF` and `WTF` to read/write from/to a file, `RFF` takes in a file path/data structure alias and a destination address, `RFF` reads in the next character and stores it in the destination address, `WTF` writes the given value to the given file at the very end of the file
